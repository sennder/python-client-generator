...
EXPECTED_PATH = Path(os.path.dirname(os.path.realpath(__file__))) / "expected/input_app"


def test_models(input_app_openapi: Dict[str, Any], tmp_path: Path) -> None:
    generate_models(input_app_openapi, tmp_path / "models.py")
...
def test_apis(input_app_openapi: Dict[str, Any], tmp_path: Path) -> None:
    generate_apis(input_app_openapi, tmp_path / "apis.py", group_by_tags=False, sync=False)
    assert filecmp.cmp(EXPECTED_PATH / "apis.py", tmp_path / "apis.py", shallow=False) is True


def test_pyproject(input_app_openapi: Dict[str, Any], tmp_path: Path) -> None:
    generate_pyproject(input_app_openapi, tmp_path / "pyproject.toml", project_name="test-project")
    assert (
...
def test_assert_openapi_version(input_app_openapi: Dict[str, Any]) -> None:
    openapi_copy = deepcopy(input_app_openapi)
